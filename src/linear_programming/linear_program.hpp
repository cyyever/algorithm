...
namespace cyy::algorithm {
  template <typename data_type = double> class linear_program {
  public:
    linear_program(directed_graph<vertex_type, weight_type> graph_,
    ...
    void max_flow_by_ford_fulkerson(
      ...
      if (!get_s_t_path_fun.has_value()) {
        get_s_t_path_fun = std::bind(
            &linear_program<vertex_type, weight_type>::get_s_t_path, this);
      }
    ...
    void max_flow_by_edmonds_karp() {
      return max_flow_by_ford_fulkerson(
          std::bind(&linear_program::get_shortest_s_t_path, this));
    }

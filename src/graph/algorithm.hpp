...
void breadth_first_search(const graph<vertex_type> &g, size_t s,
    std::function<void(edge<vertex_type>)>
                           edge_fun) {
...
template <typename vertex_type>
void depth_first_search(const graph<vertex_type> &g, size_t s,std::function<void(edge<vertex_type>)> edge_fun) {
...
auto get_depth_first_search_tree(const graph<vertex_type> &g, size_t s) {
  ...
  depth_first_search<vertex_type>(g, s,
                                    [&t](auto edge) { t.add_edge(edge); });
  return t;
}
} // namespace cyy::algorithm
